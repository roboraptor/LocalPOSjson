...
export default function PaymentSettings() {
  ...
  return (
    <div className="card h-100">
      <div className="card-header"><h5 className="mb-0">Platební QR kódy</h5></div>
              ...
              <div className="input-group has-validation">
                ...
                {ibanStatus === 'valid' && (
                  <div className="valid-feedback">
                    IBAN je v pořádku. - Váš účet: <strong>{decodeCzechIBAN(formData.bank_iban)}</strong>
                  </div>
                )}
                {ibanStatus === 'invalid' && <div className="invalid-feedback">Tento IBAN není platný.</div>}
              </div>
            </div>
            <div className="col-md-3 mb-3">
              <label className="form-label fw-bold">Konst. symbol (KS)</label>
              <input type="text" className="form-control" name="trx_ks" value={formData.trx_ks || ''} onChange={handleChange} />
            </div>
            <div className="col-md-3 mb-3">
              <label className="form-label fw-bold">Variabilní symbol (VS)</label>
              <div className="form-check form-switch mt-2">
                <input className="form-check-input" type="checkbox" id="vsEnabled" name="trx_vs_enabled" checked={formData.trx_vs_enabled} onChange={handleChange} />
                <label className="form-check-label small" htmlFor="vsEnabled">Použít číslo účtenky</label>
              </div>
            </div>
          </div>
          <div className="mb-3">
