...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({
      message: "Path updated. You may need to restart the application for changes to take effect.",
    });
  } catch (error: any) {
...
export async function PUT(request: Request) {
  try {
    ...
    if (action === "verify") {
      if (fs.existsSync(dbFullPath)) {
        try {
          ...
          return NextResponse.json(
            { error: `Soubor existuje, ale není to platná databáze: ${err.message}` },
            { status: 400 },
          );
        }
      } else {
        return NextResponse.json(
          { error: "Soubor databáze na uvedené cestě neexistuje." },
          { status: 404 },
        );
      }
    }

    // Use global database connection from getDb() to avoid connection leaks
    const db = getDb();
    ...
    if (action === "wipe") {
      // Drop all tables using active connection
      const tables = db
    ...
    if (action === "checkpoint") {
      ...
      return NextResponse.json({
        message: "Zápisy z WAL souboru byly úspěšně zapsány do databáze (WAL byl vyprázdněn).",
      });
    }
