...
export default function ExportPage() {
  ...
  // --- Checkpoint Databáze (WAL to DB) ---
  const checkpointDb = async () => {
    setLoading(true);
    try {
      ...
      if (!res.ok) throw new Error(data.error || "Nepodařilo se provést checkpoint");
      alert(data.message || "WAL soubor byl úspěšně sloučen do hlavní databáze.");
    } catch (e: any) {
      alert("Sloučení WAL selhalo: " + e.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportReceiptsCsv = async () => {
    ...
    try {
      ...
      const rows = data.map((r: any) => {
        ...
        const itemsStr = (r.items || []).map((it: any) => `${it.name} (${it.price})`).join(", ");
      ...
      downloadFile(
        csvContent,
        `uctenky_${new Date().toISOString().slice(0, 10)}.csv`,
  ...
  const exportItemsJson = async () => {
    ...
    try {
      ...
      downloadFile(
        jsonContent,
        `items_backup_${new Date().toISOString().slice(0, 10)}.json`,
        "application/json",
      );
    } catch (e: any) {
      alert("Export selhal: " + e.message);
  ...
  const exportItemsCsv = async () => {
    ...
    try {
      ...
      downloadFile(
        csvContent,
        `items_export_${new Date().toISOString().slice(0, 10)}.csv`,
        "text/csv;charset=utf-8;",
      );
    } catch (e: any) {
      alert("Export selhal: " + e.message);
    ...
    <div className="container">
      ...
      <div className="grid">
        ...
        <div className="card cardPad">
          <h2 className="card-header d-flex gap-2">
            <Fa.FaReceipt /> Účtenky
          </h2>
          <p className="text-mute">
            Stáhnout přehled všech uložených účtenek do CSV souboru (pro Excel).
          </p>
        ...
        <div className="card cardPad">
          ...
          <p className="text-mute mb-2" style={{ fontSize: "0.85em" }}>
            Pro kompletní zálohu stačí tento soubor zkopírovat.
          </p>
          <div style={{ marginTop: "1rem" }}>
            <button className="btn btn-warning d-flex align-items-center gap-2" onClick={checkpointDb} disabled={loading}>
              <Fa.FaArrowsRotate /> Sloučit WAL do DB
            </button>
          </div>
        </div>
      </div>
